...
impl From<*mut qemu::qemu_plugin_tb> for TranslationBlock {
    fn from(tb: *mut qemu::qemu_plugin_tb) -> Self {
        ...
        let count = unsafe {
            qemu::qemu_plugin_tb_n_insns(tb)
        };

        for i in 0..count {
            let insn = unsafe {
                qemu::qemu_plugin_tb_get_insn(tb as _, i)
            };

            let vaddr = unsafe {
                qemu::qemu_plugin_insn_vaddr(insn)
            };
...
impl Instruction {
    pub(crate) fn address(&self) -> u64 {
        unsafe {
            self.0.cast().read_unaligned()
        }
    }

    pub(crate) fn length(&self) -> usize {
        unsafe {
            self.0.add(std::mem::size_of::<u64>()).cast().read_unaligned()
        }
    }
    ...
    pub(crate) fn bytes(&self) -> &[u8] {
        ...
        unsafe {
            std::slice::from_raw_parts(
                self.0.add(std::mem::size_of::<u64>() + std::mem::size_of::<usize>()).as_ptr(),
                length
            )
        }
    }
