...
struct StringIndexInner {
    head: Option<Box<dyn ByteTrieNode>>,
}
...
impl StringIndexInner {
    ...
    fn search_raw_str<'a>(&'a self, string: &[u8]) -> Option<&'a dyn ByteTrieNode> {
        let mut node_ref: &dyn ByteTrieNode = self.head.as_ref().map(AsRef::as_ref)?;
        for c in string {
...
trait ByteTrieNode: Serializable {
    ...
    fn upgrade(self: Box<Self>) -> Box<dyn ByteTrieNode>;
    fn finalize(&mut self);
    ...
    fn child(&self, byte: u8) -> Option<&dyn ByteTrieNode>;
}
...
struct DenseByteTrieNode {
    ...
    trie: [Option<Box<dyn ByteTrieNode>>; 256],
}
...
struct SparseByteTrieNode {
    ...
    head: usize,
    trie_indices: [u8; 256],
    tries: [Option<Box<dyn ByteTrieNode>>; DENSE_THRESHOLD],
}
...
impl ByteTrieNode for SparseByteTrieNode {
    ...
    fn upgrade(mut self: Box<Self>) -> Box<dyn ByteTrieNode> {
...
struct VerySparseByteTrieNode {
    data: ByteTrieNodeData,
    head: usize,
    trie: [(u8, Option<Box<dyn ByteTrieNode>>); SPARSE_THRESHOLD],
}
...
impl ByteTrieNode for VerySparseByteTrieNode {
    ...
    fn insert_str(
        ...
        let mut node: Box<dyn ByteTrieNode> = Box::new(VerySparseByteTrieNode::new());
        insert_str(&mut node, depth + 1, string, data, is_prefix);
...
fn deserialize_node(bytes: &[u8], start: &mut usize) -> Box<dyn ByteTrieNode> {
    match bytes[*start] {
...
impl ByteTrieNode for DummyNode {
    ...
    fn child(&self, _byte: u8) -> Option<&dyn ByteTrieNode> {
        todo!()
    }
...
fn insert_str(
    ...
    let mut dummy: Box<dyn ByteTrieNode> = Box::new(DummyNode);
