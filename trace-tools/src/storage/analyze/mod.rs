...
pub fn analyze(input: Box<dyn Read>, outdir: PathBuf) -> Result<()> {
    ...
    let exporter = dataflow::export::csv::CsvExporter::new(&outdir);
    df.insert_exporter(exporter);

    // Setup plugins if necessary
    setup_common_plugins(&mut df, outdir.clone().into_os_string().into_string().unwrap())?;
    match df_arch {
...
fn setup_common_plugins(df: &mut dataflow::analysis::Analysis, outdir: String) -> Result<()> {
    let plugin = dataflow::plugins::cbranch::SpeculateCBranch::new();
    df.insert_plugin(Box::new(plugin));

    let plugin = dataflow::plugins::fntrack::FnTracker::make(&*df, outdir);
    df.insert_plugin(Box::new(plugin));
