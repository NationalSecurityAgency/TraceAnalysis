...
public class ExtractStatic extends GhidraScript {
	...
	for(Function f : mgr.getFunctions(true)) {
	    long fnaddr = f.getEntryPoint().getOffset()-imageBase;
	    long low = f.getBody().getMinAddress().getOffset()-imageBase;
	    long hi = f.getBody().getMaxAddress().getOffset()-imageBase;
		...
		while(itr.hasNext()) {
		    ...
		    long token_min_addr = t.getMinAddress().getOffset()-imageBase;
		    long token_max_addr = t.getMaxAddress().getOffset()-imageBase;
		...
		if(xf == null) continue;
		long xfaddr = xf.getEntryPoint().getOffset()-imageBase;
		Map<String, Object> call = new HashMap<>();
		call.put("_from",String.format("functions/%d_%d", progId, xfaddr));
		call.put("_to",String.format("functions/%d_%d", progId, fnaddr));
		call.put("callsite",String.format("%d",refs[i].getFromAddress().getOffset()-imageBase));
		calls.add(call);
	    }
		...
		long baddr = b.getMinAddress().getOffset()-imageBase;
		long bend = b.getMaxAddress().getOffset()-imageBase;
		...
		while(succs.hasNext()) {
			...
			succEntry.put("_from",String.format("blocks/%d_%d", progId, bref.getDestinationAddress().getOffset()-imageBase));
		...
		Long src = v.getCodeBlock().getFirstStartAddress().getOffset()-imageBase;
		for(CodeBlockVertex n : cdgmap.get(v)) {
		    long dst = n.getCodeBlock().getFirstStartAddress().getOffset()-imageBase;
	...
	try {
	    ...
	    FileWriter outputFile = new FileWriter(String.format("%s/%s", path, filename), true);
