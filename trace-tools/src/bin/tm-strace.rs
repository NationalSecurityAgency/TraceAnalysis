...
fn get_syscall_spec(specfile: String) -> Result<SyscallSpec> {
    ...
    let specstr =
        serde_json::from_str::<SyscallSpecStr>(String::from_utf8(specdata).unwrap().as_str())?;
...
fn coalesce(ops: Vec<&SpacetimeBlock>, addr: u64, len: usize) -> u64 {
    let mut data: Vec<TimedByte> = vec![
        TimedByte {
            val: Some(0),
            time: None
        };
        ...
        .for_each(|raw| -> ControlFlow<Error> {
            ...
            if let Record::Instruction(ref ins) = record {
                if spec
                    .opcodes
                    .iter()
                    .fold(false, |acc, opc| acc || ins.insbytes() == opc)
                {
                    let args = spec
                        .arg_regs
                        .iter()
                        .map(|reg| {
                            coalesce(
                                reg_index
                                    .find(tick - 1, *reg, *reg + spec.reg_width)
                                    .iter()
                                    .map(|block| block.as_ref())
                                    .collect::<Vec<_>>(),
                                *reg,
                                spec.reg_width as usize,
                            )
                        })
                        .collect::<Vec<u64>>();
                    let mut num: Option<u64> = None;
                    let mut ret: Option<u64> = None;
                    if let Some(numblock) = reg_index
                        .find(tick - 1, spec.num_reg, spec.num_reg + spec.reg_width)
                        .first()
                    {
                        num = Some(data_to_u64(numblock.as_ref().data.clone()));
                    }
                    if let Some(retblock) = reg_index
                        .find(tick + 1, spec.ret_reg, spec.ret_reg + spec.reg_width)
                        .first()
                    {
