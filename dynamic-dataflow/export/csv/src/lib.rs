...
impl CsvExporter {
    pub fn new<P: AsRef<Path>>(root: P) -> Self {
        ...
        let out = Writer::from_path(root.join("ticks.csv")).unwrap();
        threads.push(thread::spawn(move || write_ticks(chan, out)));

        let (delta_writer, chan) = mpsc::channel();
        let out = Writer::from_path(root.join("deltas.csv")).unwrap();
        threads.push(thread::spawn(move || write_deltas(chan, out)));

        let (addr_dep_writer, chan) = mpsc::channel();
        let out = Writer::from_path(root.join("addr_deps.csv")).unwrap();
        threads.push(thread::spawn(move || write_addr_deps(chan, out)));

        let (value_dep_writer, chan) = mpsc::channel();
        let out = Writer::from_path(root.join("input_deps.csv")).unwrap();
        threads.push(thread::spawn(move || write_value_deps(chan, out)));

        let (const_dep_writer, chan) = mpsc::channel();
        let const_out = Writer::from_path(root.join("const_uses.csv")).unwrap();
        let value_out = Writer::from_path(root.join("const_input_deps.csv")).unwrap();
        let addr_out = Writer::from_path(root.join("const_addr_deps.csv")).unwrap();
        threads.push(thread::spawn(move || {
            write_const_deps(chan, const_out, value_out, addr_out)
        }));

        let (cf_dep_writer, chan) = mpsc::channel();
        let out = Writer::from_path(root.join("cf_deps.csv")).unwrap();
...
impl DataflowExport for CsvExporter {
...
fn write_ticks(chan: Receiver<TickMessage>, mut out: Writer<File>) {
    #[derive(Serialize)]
    struct TickRecord<'a> {
        #[serde(rename = "_key")]
        key: &'a str,
            ...
            DeltaMessage::Deltas(mut deltas) => {
                for (index, tick, opc, delta) in deltas.drain(..) {
                    ...
                    if let Some(range) = Delta::associated_range(&delta) {
                        ...
                        record.assocd_addr = Some(range.offset() as _);
...
fn write_addr_deps(chan: Receiver<AddrDepMessage>, mut out: Writer<File>) {
    #[derive(Serialize)]
    struct DepRecord<'a> {
        #[serde(rename = "_from")]
        from: &'a str,
...
fn write_value_deps(chan: Receiver<ValueDepMessage>, mut out: Writer<File>) {
    #[derive(Serialize)]
    struct DepRecord<'a> {
        #[serde(rename = "_from")]
        from: &'a str,
...
fn write_cf_deps(chan: Receiver<CfDepMessage>, mut out: Writer<File>) {
    #[derive(Serialize)]
    struct DepRecord<'a> {
        #[serde(rename = "_from")]
        from: &'a str,
        ...
        match msg {
            ConstDepMessage::ValueDeps(mut deps) => {
                for (ConstEdge(index, value), pos) in deps.drain(..) {
                    ...
                    write!(raw_buffer, "{}", PartialPrinter(raw)).unwrap();
                    record.raw = Some(raw_buffer.as_str());
                    let _ = const_out.serialize(record);
                    ...
                    let _ = value_out.serialize(record);
            ...
            ConstDepMessage::AddrDeps(mut deps) => {
                for ConstEdge(index, value) in deps.drain(..) {
                    ...
                    let _ = addr_out.serialize(record);
